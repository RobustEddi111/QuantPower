...
OUT_DIR  = r"D:\工作\特变电工\13项目\储能充放电节点收益分析\储能节点收益分析\山西2025年节电电价数据储能收益测算-按月分时-不考虑四项费用"
# =================================
...
# ---------四项费用（元/MWh）
transmission_fee =65.8;        #输配电费
system_operation_fee =28.585;   # 系统运行费折价
government_fund = 22.925;        # 政府性基金及附加
line_loss_fee = 7.919;          # 上网环节线损电价
add_cost_yuan_per_mwh = transmission_fee + system_operation_fee + government_fund + line_loss_fee #四项费用合计
buy_fee  = 0*add_cost_yuan_per_mwh   # 充电侧加费
...
def optimize_profile_gurobi(prices_mwh_96):
    ...
    m.setObjective(
        gp.quicksum(
            (prices_mwh_96[t]-sell_fee) * (p_dis[t] * DT_H)
...
def main():
    ...
    out_xlsx = os.path.join(OUT_DIR, f"储能节点收益_TOP{TOPN}_月策略口径_日前实时对比_不考虑四项费用.xlsx")
    make_investment_outputs_monthly(monthly_summary, out_xlsx, topn=TOPN)

    # 另存：月策略96点到Excel（有时很大，单独一个文件更清爽）
    out_strategy_xlsx = os.path.join(OUT_DIR, "月策略_96点_日前实时_不考虑四项费用.xlsx")
